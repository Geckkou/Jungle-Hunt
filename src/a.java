...
public class a extends Applet implements Runnable {
  ...
  public void run() {
    ...
    while(true) {
	  ...
	  g.drawString("Jungle Hunt", 2, 15);
	  
	  g.setColor(Color.WHITE);
	  g.drawString("Record: " + recorde ,2, 190);
	  
	  if(cheat) {
		  g.setColor(Color.ORANGE);
		  g.drawString("Cheat", 102, 190);
	  }
      // -- render ends --------------------------------------------------------
  ...
  public void processKeyEvent(KeyEvent keyEvent) {
    ...
    final int VK_LIGA_TRAPACA = 84; //t
    final int VK_DESLIGA_TRAPACA = 85; //u
    ...
    if (k > 0) {
      k = k == VK_W ? VK_UP : k == VK_D ? VK_RIGHT : k == VK_A ? VK_LEFT
          : k == VK_S || k == VK_LIGA_TRAPACA || k == VK_DESLIGA_TRAPACA ? VK_DOWN : k;
